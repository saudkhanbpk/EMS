    ...
    const sendEmail = async () => {
      try {
        const response = await fetch(
          'https://ems-server-0bvq.onrender.com/send-email',
          {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
              ...
              subject: `New Leave Request Submitted by ${localStorage.getItem(
                'user_email'
              )}`,
        ...
        if (response.ok) {
          ...
          toast.success('Failed to send email', data.error);
        }
      } catch (error) {
    ...
    <div className="w-full">
      <div className="mb-4">
        <button
          ...
          className="text-gray-600 hover:bg-gray-300 rounded-2xl
         translate-x-2 transition-transform ease-in-out duration-300 transform hover:scale-205"
        >
          <ChevronLeft />
        </button>
      </div>
      <h2 className="text-[28px] font-bold mb-6 leading-9 text-[#000000]">
        Leaves Request
      </h2>

      <div className="max-w-50 mx-auto mt-8 p-6 bg-[#FFFFFF] rounded-lg shadow-lg">
        <h2 className="text-[22px] font-semibold mb-6 leading-7 text-[#000000]">
          Request a Leave
        </h2>
